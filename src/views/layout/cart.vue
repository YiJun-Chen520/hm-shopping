    ...
    <div v-if="isLogin && cartTotal > 0">
        ...
        <div class="cart-item" v-for="item in cartList" :key="item.goods_id">
          ...
          <div class="info">
            ...
            <span class="bottom">
              ...
              <CountBox @input="(value) => changeCount(value, item.goods_id, item.goods_sku_id)" v-model="item.goods_num"></CountBox>
            </span>
          </div>
        </div>
      </div>
      ...
      <div class="footer-fixed">
        ...
        <div class="all-total">
          ...
          <div @click="goPay" v-if="!isEdit" :class="{ disabled: selCartTotal === 0}" class="goPay">结算({{  selCartTotal }})</div>
          <div @click="handleDel" v-else :class="{ disabled: selCartTotal === 0}" class="delete">删除</div>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters('cart', ['cartTotal', 'isAllChecked', 'selCartTotal', 'selCartPrice', 'selCartList'])
  },
  created () {
    // 先判断是否登录
    if (this.isLogin) {
