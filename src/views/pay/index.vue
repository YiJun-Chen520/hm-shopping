...
<script>
import { getAddressListAPI, submitOrderAPI } from '@/api/address'
...
export default {
  ...
  methods: {
    ...
    async getOrderList () {
      if (this.mode === 'cart') {
        ...
        const { data: { order, personal } } = await checkOrderAPI(this.mode, {
  ...
  computed: {
    ...
    goodsNum () {
      return this.$route.query.goodsNum >= 1 ? this.$route.query.goodsNum : 1
    }
